...
The primary boundaries are the browser/API origin, public outbound fetch path, manifest normalization boundary, ephemeral report store, durable sanitized URL-attempt store, browser WebMCP registration surface, and raw-crawl-to-derived-index audit boundary.
...
| Sensitive data embedded in submitted URLs         | Credentials, query strings, and fragments are removed before persistence; malformed, private, and credential-bearing inputs are counted without retaining their submitted value; goals, markup, IPs, and user agents are excluded | A path segment can itself contain sensitive text. The UI warns users not to submit private or secret-bearing URLs, access to the Postgres store remains owner-only, and records are pruned after 90 days.                        |
