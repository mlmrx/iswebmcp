...
function categoryInputCoverage(
  category: ScanReport['baselineActionability']['categories'][number],
) {
  const totalPossible =
  ...
  if (!totalPossible) return category.score === null ? 0 : 100;
  ...
  return Math.round((observedPossible / totalPossible) * 100);
}

function reportSummary(report: ScanReport) {
  const provenance =
    report.reportKind === 'synthetic_fixture'
      ? 'Synthetic fixture; no target website was fetched'
      : report.response.truncated
        ? 'Observed source; bounded partial response'
        : 'Observed public source';
  const sourceScope = report.response.truncated
    ? 'captured-prefix estimate; complete-page score unknown (0–100)'
    : 'captured-source estimate';
  return `${report.normalizedUrl}\nProvenance: ${provenance}\nImplementation: ${stateLabels[report.implementationState]}\nSource actionability: ${report.baselineActionability.value ?? 'N/A'}/100 (${report.baselineActionability.coverage}% model-input coverage; ${sourceScope}; ${report.baselineActionability.modelVersion ?? 'legacy model'})\nContract lint: ${report.webmcpQuality ? `${report.webmcpQuality.value ?? 'N/A'}/100 (${report.webmcpQuality.coverage}% imported model-input coverage)` : 'Not evaluated'}\nRuntime readiness: Not measured\nWebMCP Lift: Not calculated\nTop evidence: ${report.strongestEvidence.join('; ')}\nGenerated by isWebMCP.`;
}
  ...
  return (
    ...
    >
        ...
        >
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {report.response.truncated && (
                <span className="status-chip border-warning/30 bg-warning/10 text-warning">
              ...
              <span className="font-mono text-[10px] uppercase tracking-wider text-muted-foreground">
                {report.reportKind === 'synthetic_fixture'
                  ? 'Synthetic fixture · no network observation'
                  : 'Source observation'}{' '}
                · {formatTime(report.scannedAt)}
              </span>
            </div>
      ...
      <div className="mx-auto max-w-7xl space-y-8 px-5 py-8 lg:px-8">
        ...
        {report.reportKind === 'synthetic_fixture' && (
          <aside className="rounded-xl border border-signal-ink/25 bg-signal/8 p-4 text-sm leading-6">
            <strong>Synthetic sample:</strong> this report demonstrates the
            evidence format with an authored storefront fixture. Its URL,
            source, and score are not observations of a public website.
          </aside>
        )}
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-t border-border pt-5 text-xs text-muted-foreground">
          ...
          {report.reportKind === 'synthetic_fixture' ? (
            <span className="font-semibold text-foreground">
              Synthetic URL reference · no live target
            </span>
          ) : (
            <a
